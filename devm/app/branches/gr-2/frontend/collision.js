...
function detailedTest(shape1,shape2){
    var i,diff;
    diff = vec3.subtract(shape2.getPosition(),shape1.getPosition(),vec3.create());
    var poly1,poly2;
    poly1 = shape1.getVertices();
    poly2 = shape2.getVertices();
    var polymoved = [];
    for(i=0;i<poly2.length;++i){
        polymoved.push(
                vec3.add(poly2[i],diff,vec3.create())
             );
    }
    poly2=polymoved;
    
    for(i=0;i<poly1.length;++i){
        if(crossingsMultiplyTest(poly2,poly1[i])){
            return true;
        }
    }
    for(i=0;i<poly2.length;++i){
        if(crossingsMultiplyTest(poly1,poly2[i])){
            return true;
        }
    }
...
function calculateSeparatingVelocity (a, b) { 
  var relativeVelocity = vec3.subtract( a.getVelocity(), b.getVelocity(), vec3.create()); 
  return vec3.dot( relativeVelocity, 
            vec3.normalize( 
                vec3.subtract( a.getPosition(), b.getPosition(), vec3.create())));
}
...
function resolveVelocity (a, b) { 
  ...
  var totalInversMass = a.getInverseMass() + b.getInverseMass(); // watch out on infinite masses
  var impulse = deltaVelocity / totalInverseMass; 
  var contactNormal = vec3.normalize(
                          vec3.subtract(a.getPosition(), b.getPosition(),vec3.create()));

  var impulsePerMass = vec3.scale(contactNormal, impulse);
  vec3.add(a.getVelocity(), 
      vec3.scale(impulsePerMass, a.getInverseMass(), vec3.create())); 
  
  vec3.add(b.getVelocity(), 
      vec3.scale(impulsePerMass, -b.getInverseMass(), vec3.create()));
}
